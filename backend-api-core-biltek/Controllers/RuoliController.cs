...
namespace backend_api_core.Controllers
{
    ...
    {
        private readonly IRuoli _ruoli;

        public RuoliController(IRuoli ruoli)
        {
            this._ruoli = ruoli;
        }
        ...
        public async Task<ActionResult<IEnumerable<RespuestaCore>>> Get(string value, string parameter, int currentPageNumber, int amountShow)
        {
            ...
            IEnumerable<RuoliList> _arrayLista = new RuoliList[] { };
            int elementosTotales = 0;

            try
            {
                _arrayLista = await this._ruoli.SearchList(value, parameter, currentPageNumber, amountShow);
        ...
        public async Task<ActionResult<RespuestaCore>> Get(int id)
        {
            ...
            try
            {
                datos = await this._ruoli.FindById(id);
        ...
        // POST: api/Modello
        [HttpPost]
        public async Task<ActionResult<object>> Post([FromBody] Ruoli data)
        {
            RespuestaCore respuesta = new RespuestaCore();
            ...
            try
            {
                ...
                {
                    respuestaBD = await this._ruoli.Save(data);
        ...
        [HttpPut]
        public async Task<ActionResult<object>> Put([FromBody] Ruoli data)
        {
            RespuestaCore respuesta = new RespuestaCore();
            RespuestaDB respuestaBD = new RespuestaDB();
            ...
            try
            {
                using (TransactionScope transaction = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                {
                    respuestaBD = await this._ruoli.Modify(data);
                    if (respuestaBD.status == "error")
                    {
                        return new OkObjectResult(respuestaBD);
                    }
        ...
        public async Task<ActionResult<RespuestaDB>> Delete(int id)
        {
            ...
            try
            {
                using (TransactionScope transaction = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                {
                    respuestaBD = await this._ruoli.Delete(id);
                    if (respuestaBD.status == "error")
                    {
                        return new OkObjectResult(respuestaBD);
                    }
