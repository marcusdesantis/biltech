...
namespace backend_api_core.Controllers
{
    ...
    public class FormulaController : ControllerBase
    {
        ...
        public FormulaController(IFormula formula)
        {
        ...
        public async Task<ActionResult<IEnumerable<RespuestaCore>>> Get(string value, string parameter, int currentPageNumber, int amountShow)
        {
            ...
            IEnumerable<FormulaList> _arrayLista = new FormulaList[] { };
            int elementosTotales = 0;

            try
            {
                _arrayLista = await this._formula.SearchList(value, parameter, currentPageNumber, amountShow);
        ...
        public async Task<ActionResult<RespuestaCore>> Get(int id)
        {
            ...
            try
            {
                datos = await this._formula.FindById(id);
        ...
        // POST: api/Formula
        [HttpPost]
        public async Task<ActionResult<object>> Post([FromBody] Formula data)
        {
            RespuestaCore respuesta = new RespuestaCore();
            ...
            try
            {
                ...
                {
                    respuestaBD = await this._formula.Save(data);
        ...
        // PUT api/Formula (MODIFICAR)
        [HttpPut]
        public async Task<ActionResult<object>> Put([FromBody] Formula data)
        {
            RespuestaCore respuesta = new RespuestaCore();
            RespuestaDB respuestaBD = new RespuestaDB();
            ...
            try
            {
                ...
                using (TransactionScope transaction = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                {
                    respuestaBD = await this._formula.Modify(data);
                    if (respuestaBD.status == "error")
                    {
                        return new OkObjectResult(respuestaBD);
                    }
        ...
        public async Task<ActionResult<RespuestaDB>> Delete(int id)
        {
            ...
            try
            {
                using (TransactionScope transaction = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                {
                    respuestaBD = await this._formula.Delete(id);
                    if (respuestaBD.status == "error")
                    {
                        return new OkObjectResult(respuestaBD);
                    }
