...
namespace backend_api_core.Controllers
{
    ...
    [ApiController]
    public class ControlloPesoController : ControllerBase
    {
        private readonly IControlloPeso _controlloPeso;

        public ControlloPesoController(IControlloPeso controlloPeso)
        {
            this._controlloPeso = controlloPeso;
        }
        ...
        public async Task<ActionResult<IEnumerable<RespuestaCore>>> Get(string value, string parameter, int currentPageNumber, int amountShow)
        {
            ...
            IEnumerable<ControlloPesoList> _arrayLista = new ControlloPesoList[] { };
            int elementosTotales = 0;

            try
            {
                _arrayLista = await this._controlloPeso.SearchList(value, parameter, currentPageNumber, amountShow);
        ...
        public async Task<ActionResult<RespuestaCore>> Get(int id)
        {
            ...
            try
            {
                datos = await this._controlloPeso.FindById(id);
        ...
        // POST: api/Modello
        [HttpPost]
        public async Task<ActionResult<object>> Post([FromBody] ControlloPeso data)
        {
            RespuestaCore respuesta = new RespuestaCore();
            ...
            try
            {
                ...
                {
                    respuestaBD = await this._controlloPeso.Save(data);
        ...
        [HttpPut]
        public async Task<ActionResult<object>> Put([FromBody] ControlloPeso data)
        {
            RespuestaCore respuesta = new RespuestaCore();
            RespuestaDB respuestaBD = new RespuestaDB();
            ...
            try
            {
                using (TransactionScope transaction = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                {
                    respuestaBD = await this._controlloPeso.Modify(data);
                    if (respuestaBD.status == "error")
                    {
                        return new OkObjectResult(respuestaBD);
                    }
        ...
        public async Task<ActionResult<RespuestaDB>> Delete(int id)
        {
            ...
            try
            {
                using (TransactionScope transaction = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                {
                    respuestaBD = await this._controlloPeso.Delete(id);
                    if (respuestaBD.status == "error")
                    {
                        return new OkObjectResult(respuestaBD);
                    }
