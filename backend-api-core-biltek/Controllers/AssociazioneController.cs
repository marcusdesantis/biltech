...
namespace backend_api_core.Controllers
{
    ...
    {
        private readonly IAssociazione _associazione;

        public AssociazioneController(IAssociazione associazione)
        {
            this._associazione = associazione;
        }
        ...
        public async Task<ActionResult<IEnumerable<RespuestaCore>>> Get(string value, string parameter, int currentPageNumber, int amountShow)
        {
            ...
            IEnumerable<AssociazioneList> _arrayLista = new AssociazioneList[] { };
            int elementosTotales = 0;

            try
            {
                _arrayLista = await this._associazione.SearchList(value, parameter, currentPageNumber, amountShow);
        ...
        public async Task<ActionResult<RespuestaCore>> Get(int id)
        {
            ...
            try
            {
                datos = await this._associazione.FindById(id);
        ...
        // POST: api/Modello
        [HttpPost]
        public async Task<ActionResult<object>> Post([FromBody] Associazione data)
        {
            RespuestaCore respuesta = new RespuestaCore();
            ...
            try
            {
                ...
                {
                    respuestaBD = await this._associazione.Save(data);
        ...
        [HttpPut]
        public async Task<ActionResult<object>> Put([FromBody] Associazione data)
        {
            RespuestaCore respuesta = new RespuestaCore();
            RespuestaDB respuestaBD = new RespuestaDB();
            ...
            try
            {
                using (TransactionScope transaction = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                {
                    respuestaBD = await this._associazione.Modify(data);
                    if (respuestaBD.status == "error")
                    {
                        return new OkObjectResult(respuestaBD);
                    }
        ...
        public async Task<ActionResult<RespuestaDB>> Delete(int id)
        {
            ...
            try
            {
                using (TransactionScope transaction = new TransactionScope(TransactionScopeAsyncFlowOption.Enabled))
                {
                    respuestaBD = await this._associazione.Delete(id);
                    if (respuestaBD.status == "error")
                    {
                        return new OkObjectResult(respuestaBD);
                    }
